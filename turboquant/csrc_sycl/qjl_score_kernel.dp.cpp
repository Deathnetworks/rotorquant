...
SYCL_EXTERNAL __inline__ float warpReduceSum(float val) {
  for (int offset = sycl::ext::oneapi::this_work_item::get_sub_group()
       ...
       offset > 0; offset >>= 1)
    /*
    DPCT1121:9: Make sure that the "val" which is used in the SYCL group
...
calc_score_kernel exceeds 128 bytes and may cause high register pressure.
    ...
    */
    ...
    for (size_t chnl_tile{0}; chnl_tile < sketch_dim; chnl_tile += (8*WARP_SIZE)){
        ...
        for (size_t grp_tile{wIdx}; grp_tile < WARP_SIZE; grp_tile += WARPS_PER_BLOCK) {
            ...
            /*
            DPCT1118:12: SYCL group functions and algorithms must be encountered
            ...
            */
            ...
            /*
            DPCT1118:13: SYCL group functions and algorithms must be encountered
            in converged control flow. You may need to adjust the code.
            */
            /*
            ...
            */
            ...
            /*
            DPCT1118:14: SYCL group functions and algorithms must be encountered
            in converged control flow. You may need to adjust the code.
            */
            /*
...
torch::Tensor QJLScoreCudaTemplate(
    ...
    dpct::dim3 numBlocks(batch * head, n, blocksPerGroup);
