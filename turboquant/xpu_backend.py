"""
XPU backend for TurboQuant.

Wraps the QJL and RotorQuant XPU kernels with a Python-friendly interface.
...
def qjl_score(key_quant, key_outlier_quant, key_norm, key_outlier_norm,
    ...
    return getattr(xpu_qjl_score, fn_name)(
...
def qjl_gqa_score(key_quant, key_outlier_quant, key_norm, key_outlier_norm,
    ...
    if fn_name is None:
        raise TypeError(f"Unsupported dtypes: query={query_dtype}, proj={rand_dtype}")
    return getattr(xpu_qjl_gqa_score, fn_name)(
        key_quant, key_outlier_quant, key_norm, key_outlier_norm,
        outlier_indices, query_sketch, query_states, rand_prj)
...
def quantized_bmm(group_size, fA, qB, scales, zeros, bits, mqa=False):
    ...
    result = getattr(xpu_quantization, fn_name)(fA, qB, scales, zeros, bits, group_size, nh, mqa)
...
class QJLSketch(nn.Module):
    def __init__(self, dim: Tuple[int, int], dim_outlier: int,
        ...
        super().__init__()
        self.device = device or torch.device('xpu' if torch.xpu.is_available() else 'cpu')
    ...
    def quantize_xpu(self, data, outlier_indices):
    ...
    def quantize(self, data, outlier_indices):
        if _XPU_AVAILABLE and data.is_xpu:
            return self.quantize_xpu(data, outlier_indices)
        else:
    ...
    def calc_score_xpu(self, query, data_quant, outlier_quant, outlier_indices, norm_data, norm_outlier):
    ...
    def calc_score(self, query, data_quant, outlier_quant, outlier_indices, norm_data, norm_outlier):
        if _XPU_AVAILABLE and query.is_xpu:
            return self.calc_score_xpu(query, data_quant, outlier_quant, outlier_indices, norm_data, norm_outlier)
        raise RuntimeError("XPU kernels required for calc_score. Build with: python setup_xpu.py build_ext --inplace")
