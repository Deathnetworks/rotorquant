"""
Benchmark: TurboQuant PyTorch vs QJL XPU kernels.

Compares:
1. Pure PyTorch QJL (sign(S @ residual)) - Stage 2 of TurboQuant
2. XPU fused QJL quantization + score computation
3. Full TurboQuant pipeline: Lloyd-Max (Stage 1) + QJL XPU (Stage 2)

Usage:
    python -m turboquant.benchmark_xpu
"""
...
from turboquant.xpu_backend import is_xpu_available, QJLSketch, QJLKeyQuantizer
...
def benchmark_qjl_quantize(device="xpu"):
    """Compare QJL quantization: PyTorch vs XPU kernel."""
    print("=" * 70)
    ...
    for seq_len in [1024, 4096, 8192, 16384]:
        ...
        if is_xpu_available():
            ...
            for _ in range(n_iter):
                kq, koq, kon = sketch.quantize(keys, outlier_idx)
            torch.xpu.synchronize()
            xpu_ms = (time.perf_counter() - t0) / n_iter * 1000
            speedup = pytorch_ms / xpu_ms
            print(f"  seq={seq_len:>6d}: PyTorch={format_time(pytorch_ms):>10s}  "
                  f"XPU={format_time(xpu_ms):>10s}  speedup={speedup:.1f}x")
        else:
            print(f"  seq={seq_len:>6d}: PyTorch={format_time(pytorch_ms):>10s}  "
                  f"XPU=N/A (kernels not built)")

    print()


def benchmark_attention_scores(device="xpu"):
    ...
    for seq_len in [1024, 4096, 8192]:
        ...
        if is_xpu_available():
            ...
            print(f"  seq={seq_len:>6d}: FP16={format_time(fp_ms):>10s}  "
                  f"XPU-QJL={format_time(xpu_ms):>10s}  "
                  f"speedup={fp_ms / xpu_ms:.1f}x  "
...
def benchmark_e2e_pipeline(device="xpu"):
...
def benchmark_accuracy_comparison(device="xpu"):
    """Compare accuracy: PyTorch QJL vs XPU kernel QJL."""
    print("=" * 70)
    print("BENCHMARK 4: Accuracy - PyTorch vs XPU (inner product fidelity)")
    print("=" * 70)

    if not is_xpu_available():
        print("  XPU kernels not available, skipping accuracy comparison.")
        print()
        return
    ...
    quantizer = QJLKeyQuantizer(sketch, outliers_count=outliers_count,
                                ...
                                qjl_dim=sketch_dim)
    quantizer.build_sketch(keys)
    scores_xpu = quantizer.attention_score(query).squeeze(-2)  # (B, H, S)
    ...
    for b in range(batch):
        for h in range(heads):
            fp = scores_fp[b, h]
            xpu = scores_xpu[b, h, :fp.shape[0]]
            cos = F.cosine_similarity(fp.unsqueeze(0).float(), xpu.unsqueeze(0).float()).item()
            cos_sims.append(cos)
            if fp.argmax() == xpu.argmax():
                top1_matches += 1
            n_checks += 1
...
if __name__ == "__main__":
    ...
    print()
    print("TurboQuant XPU Benchmark")
    print(f"GPU: {torch.xpu.get_device_name()}")
    print(f"XPU kernels available: {is_xpu_available()}")
    print()
