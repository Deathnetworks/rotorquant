...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_ISO4] = {
        .type_name                = "iso4",
        .blck_size                = QK4_0,
        .type_size                = sizeof(block_iso4),
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_iso4,
        .from_float_ref           = (ggml_from_float_t) quantize_row_iso4_ref,
    },
    [GGML_TYPE_ROTOR4] = {
        .type_name                = "rotor4",
        .blck_size                = QK4_0,
        .type_size                = sizeof(block_rotor4),
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_rotor4,
        .from_float_ref           = (ggml_from_float_t) quantize_row_rotor4_ref,
    },
};

const struct ggml_type_traits * ggml_get_type_traits(enum ggml_type type) {
    if (type < 0 || type >= GGML_TYPE_COUNT) {
        fprintf(stderr, "ggml_get_type_traits: bad type %d\n", (int)type); fflush(stderr);
    }
    assert(type >= 0);
...
size_t ggml_row_size(enum ggml_type type, int64_t ne) {
    ...
    if (ne % ggml_blck_size(type) != 0) {
        fprintf(stderr, "ggml_row_size assertion failed: ne (%" PRId64 ") %% ggml_blck_size(%d) != 0 for type %s\n",
                (int64_t)ne, (int)ggml_blck_size(type), ggml_type_name(type));
        assert(false);
    }
...
struct ggml_context * ggml_init(struct ggml_init_params params) {
    fprintf(stderr, "ggml_init: start (mem_size=%zu)\n", params.mem_size); fflush(stderr);
...
size_t ggml_quantize_chunk(
    ...
    switch (type) {
        ...
        case GGML_TYPE_ISO4:    result = quantize_iso4   (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
        case GGML_TYPE_ROTOR4:  result = quantize_rotor4 (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
        case GGML_TYPE_F16:
            {
