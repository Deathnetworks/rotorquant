...
// Clifford Algebra Cl(3,0) sparse geometric product helper functions for RotorQuant
static void gp_rotor_mv_cpu(float s, float p12, float p13, float p23, const float x[8], float r[8]) {
...
static void gp_mv_rotor_cpu(const float x[8], float s, float p12, float p13, float p23, float r[8]) {
    r[0] = s*x[0] - p12*x[4] - p13*x[5] - p23*x[6];
    r[1] = s*x[1] - p12*x[2] - p13*x[3] + p23*x[7];
    r[2] = s*x[2] + p12*x[1] - p23*x[3] - p13*x[7];
    r[3] = s*x[3] + p13*x[1] + p23*x[2] + p12*x[7];
...
static void rotor_sandwich_vec_cpu(float s, float p12, float p13, float p23, const float x_in[3], float y_out[3]) {
    float x_mv[8] = {0, x_in[0], x_in[1], x_in[2], 0, 0, 0, 0};
    float tmp[8];
    gp_rotor_mv_cpu(s, p12, p13, p23, x_mv, tmp);
    float res[8];
    gp_mv_rotor_cpu(tmp, s, -p12, -p13, -p23, res);
    y_out[0] = res[1];
    y_out[1] = res[2];
    y_out[2] = res[3];
}

static float get_next_lcg(uint32_t * x) {
    *x = *x * 1664525u + 1013904223u;
    return (float)*x / 4294967296.0f;
}

static void get_deterministic_rotor_cpu(int group_idx, float * s, float * p12, float * p13, float * p23) {
    ...
    float norm = sqrtf(v1*v1 + v2*v2 + v3*v3 + 1e-8f);
...
// 4D Quaternion helpers for ISO4
static void quat_multiply_left_cpu(float s, float qx, float qy, float qz, const float v[4], float r[4]) {
    r[0] = s*v[0] - qx*v[1] - qy*v[2] - qz*v[3];
    r[1] = s*v[1] + qx*v[0] + qy*v[3] - qz*v[2];
    r[2] = s*v[2] - qx*v[3] + qy*v[0] + qz*v[1];
    r[3] = s*v[3] + qx*v[2] - qy*v[1] + qz*v[0];
}

static void get_deterministic_quaternion_cpu(int group_idx, float * s, float * x, float * y, float * z) {
    ...
    float norm = sqrtf(q0*q0 + q1*q1 + q2*q2 + q3*q3 + 1e-8f);
...
void quantize_row_iso4_ref(const float * GGML_RESTRICT x, block_iso4 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int g = 0; g < 8; g++) {
            float s, qx, qy, qz;
            get_deterministic_quaternion_cpu(i*8 + g, &s, &qx, &qy, &qz);
            float quad[4] = {x[i*qk + g*4], x[i*qk + g*4 + 1], x[i*qk + g*4 + 2], x[i*qk + g*4 + 3]};
            float res[4];
            quat_multiply_left_cpu(s, qx, qy, qz, quad, res);
            for (int l = 0; l < 4; l++) rotated[g*4 + l] = res[l];
        }

        float amax = 0.0f;
        for (int j = 0; j < qk; j++) amax = MAX(amax, fabsf(rotated[j]));
        ...
        for (int j = 0; j < qk/2; ++j) {
            ...
            for (int l = 0; l < 16; ++l) {
                float dist0 = fabsf(x0 - iso4_normalized_centroids[l]);
                if (dist0 < mindist0) { mindist0 = dist0; xi0 = l; }
                float dist1 = fabsf(x1 - iso4_normalized_centroids[l]);
...
void quantize_row_rotor4_ref(const float * GGML_RESTRICT x, block_rotor4 * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK4_0;
    assert(k % qk == 0);
    const int nb = k / qk;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int g = 0; g < 10; g++) {
            float s, p12, p13, p23;
            get_deterministic_rotor_cpu(i*10 + g, &s, &p12, &p13, &p23);
            float triple[3] = {x[i*qk + g*3], x[i*qk + g*3+1], x[i*qk + g*3+2]};
            float res[3];
            rotor_sandwich_vec_cpu(s, p12, p13, p23, triple, res);
            rotated[g*3]   = res[0];
            rotated[g*3+1] = res[1];
            rotated[g*3+2] = res[2];
        }
        // Leftovers
        rotated[30] = x[i*qk + 30];
        rotated[31] = x[i*qk + 31];
        ...
        for (int j = 0; j < qk/2; ++j) {
            ...
            uint8_t xi0 = 0;
            uint8_t xi1 = 0;
            float mindist0 = fabsf(x0 - iso4_normalized_centroids[0]);
            float mindist1 = fabsf(x1 - iso4_normalized_centroids[0]);
...
void dequantize_row_iso4(const block_iso4 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    static const int qk = 32;
    assert(k % qk == 0);
    const int nb = k / qk;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < qk/2; ++j) {
            const int xi0 = x[i].qs[j] & 0x0F;
            const int xi1 = x[i].qs[j] >> 4;
            dequantized[2*j]     = d * iso4_normalized_centroids[xi0];
            dequantized[2*j + 1] = d * iso4_normalized_centroids[xi1];
        }

        // Apply inverse 4D quaternions to 8 groups of 4
        for (int g = 0; g < 8; g++) {
            float s, qx, qy, qz;
            get_deterministic_quaternion_cpu(i*8 + g, &s, &qx, &qy, &qz);
            float quad[4] = {dequantized[g*4], dequantized[g*4 + 1], dequantized[g*4 + 2], dequantized[g*4 + 3]};
            float res[4];
            // Inverse is conjugate: negate x, y, z
            quat_multiply_left_cpu(s, -qx, -qy, -qz, quad, res);
            if (i == 0 && g == 0) {
                printf("CPU block 0 g 0: d=%f, s=%f, qx=%f, qy=%f, qz=%f, quad0=%f, res0=%f\n", d, s, qx, qy, qz, quad[0], res[0]);
            }
            for (int l = 0; l < 4; l++) y[i*qk + g*4 + l] = res[l];
        }
    }
}


void dequantize_row_rotor4(const block_rotor4 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK4_0;
    assert(k % qk == 0);
    const int nb = k / qk;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        // Apply inverse rotors to 10 groups of 3
        for (int g = 0; g < 10; g++) {
            float s, p12, p13, p23;
            get_deterministic_rotor_cpu(i*10 + g, &s, &p12, &p13, &p23);
            float triple[3] = {dequantized[g*3], dequantized[g*3+1], dequantized[g*3+2]};
            float res[3];
            // Inverse rotation is sandwich with reverse rotor (negate bivector parts)
            rotor_sandwich_vec_cpu(s, -p12, -p13, -p23, triple, res);
            if (i == 0 && g == 0) {
                printf("CPU block 0 g 0 (ROTOR4): d=%f, s=%f, p12=%f, p13=%f, p23=%f, triple0=%f, res0=%f\n", d, s, p12, p13, p23, triple[0], res[0]);
            }
            y[i*qk + g*3]   = res[0];
            y[i*qk + g*3+1] = res[1];
            y[i*qk + g*3+2] = res[2];
        ...
        // Leftovers
        y[i*qk + 30] = dequantized[30];
        y[i*qk + 31] = dequantized[31];
    }
}
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_ISO4:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_iso4, data, nb);
            } break;
        case GGML_TYPE_ROTOR4:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_rotor4, data, nb);
            } break;
...
size_t quantize_iso4(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    const size_t row_size = ggml_row_size(GGML_TYPE_ISO4, n_per_row);
    quantize_row_iso4_ref(src, (block_iso4 *)dst, nrow * n_per_row);
    return nrow * row_size;
}

size_t quantize_rotor4(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    const size_t row_size = ggml_row_size(GGML_TYPE_ROTOR4, n_per_row);
    quantize_row_rotor4_ref(src, (block_rotor4 *)dst, nrow * n_per_row);
    return nrow * row_size;
}
