...
static __dpct_inline__ void dequantize_iso4(const void *vx, const int64_t ib,
                                            const int iqs, dfloat2 &v) {
    const block_iso4 * x = (const block_iso4 *) vx;
    const float d = (float)x[ib].d;

    // iqs handles 2 elements at once
    int g0 = (iqs * 2) / 4;
    int i0 = (iqs * 2) % 4;
    int g1 = (iqs * 2 + 1) / 4;
    int i1 = (iqs * 2 + 1) % 4;

    float s0, qx0, qy0, qz0;
    get_deterministic_quaternion(ib * 8 + g0, &s0, &qx0, &qy0, &qz0);

    float c0[4];
    for (int j = 0; j < 4; ++j) {
        int idx = g0 * 4 + j;
        int ki = (x[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
        c0[j] = d * iso4_normalized_centroids[ki];
    }
    float w0[4];
    quat_multiply_left(s0, -qx0, -qy0, -qz0, c0, w0);
    v.x() = w0[i0];

    if (g0 == g1) {
        v.y() = w0[i1];
    } else {
        float s1, qx1, qy1, qz1;
        get_deterministic_quaternion(ib * 8 + g1, &s1, &qx1, &qy1, &qz1);
        float c1[4];
        for (int j = 0; j < 4; ++j) {
            int idx = g1 * 4 + j;
            int ki = (x[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
            c1[j] = d * iso4_normalized_centroids[ki];
        }
        float w1[4];
        quat_multiply_left(s1, -qx1, -qy1, -qz1, c1, w1);
        v.y() = w1[i1];
    }
}

static __dpct_inline__ void dequantize_rotor4(const void *vx, const int64_t ib,
                                            const int iqs, dfloat2 &v) {
    ...
    for (int l = 0; l < 2; ++l) {
        ...
        if (idx < 30) {
            ...
            for (int j = 0; j < 3; ++j) {
                int c_idx = g * 3 + j;
                int ki = (x[ib].qs[c_idx/2] >> (4 * (c_idx%2))) & 0x0F;
                c[j] = d * rotor4_normalized_centroids[ki];
            }
            float w[3];
            rotor_sandwich_vec(s, -p12, -p13, -p23, c, w);
            val = w[i];
        } else {
            int ki = (x[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
            val = d * rotor4_normalized_centroids[ki];
        }
        if (l == 0) v.x() = val; else v.y() = val;
    }
}
...
template<typename dst_t>
static void dequantize_block_iso4(const void * __restrict__ vx, dst_t * __restrict__ yy,
                                  const int64_t nb_row, const sycl::nd_item<3> &item_ct1) {
    const int64_t i_row = item_ct1.get_group(1);
    const int64_t i_local = item_ct1.get_group(2);
    const int64_t ib = i_row * nb_row + i_local;
    const block_iso4 * x = (const block_iso4 *) vx + ib;
    ...
    for (int j = 0; j < 4; ++j) {
        int idx = g * 4 + j;
        int ki = (x->qs[idx/2] >> (4 * (idx%2))) & 0x0F;
        c[j] = d * iso4_normalized_centroids[ki];
    }
    ...
    float w_rot[4];
    quat_multiply_left(s, -qx, -qy, -qz, c, w_rot);

    yy[ib * 32 + tid] = (dst_t)w_rot[i];
}

template<typename dst_t>
static void dequantize_block_rotor4(const void * __restrict__ vx, dst_t * __restrict__ yy,
                                  const int64_t nb_row, const sycl::nd_item<3> &item_ct1) {
    const int64_t i_row = item_ct1.get_group(1);
    const int64_t i_local = item_ct1.get_group(2);
    const int64_t ib = i_row * nb_row + i_local;
    const block_rotor4 * x = (const block_rotor4 *) vx + ib;
    const float d = (float)x->d;
    ...
    if (tid < 30) {
        ...
        float w_rot[3];
        rotor_sandwich_vec(s, -p12, -p13, -p23, c, w_rot);
        
        yy[ib * 32 + tid] = (dst_t)w_rot[i];
    } else {
        int ki = (x->qs[tid/2] >> (4 * (tid%2))) & 0x0F;
        yy[ib * 32 + tid] = (dst_t)(d * rotor4_normalized_centroids[ki]);
    }
}
