...
static ggml_sycl_device_info ggml_sycl_init() {
    ...
    try {
        // Native SYCL discovery (Bypassing dpct::dev_mgr for enumeration)
        auto platforms = sycl::platform::get_platforms();
        
        // Prioritize Level Zero for performance and stability on Arc Pro B70
        sycl::platform * preferred_platform = nullptr;
        ...
        if (preferred_platform) {
            gpu_devices = preferred_platform->get_devices(sycl::info::device_type::gpu);
        } else {
            // Fallback to all GPUs if Level Zero not found
            gpu_devices = sycl::device::get_devices(sycl::info::device_type::gpu);
        }
        
        info.device_count = (int)gpu_devices.size();

        // If native discovery is empty, attempt dpct as secondary fallback
        if (info.device_count == 0) {
            info.device_count = dpct::dev_mgr::instance().device_count();
            for (int i = 0; i < info.device_count; ++i) {
                gpu_devices.push_back(dpct::dev_mgr::instance().get_device(i));
            }
        }
    } catch (const std::exception & e) {
        fprintf(stderr, "ggml_sycl_init: fail-safe triggered (%s), returning 0 devices\n", e.what()); fflush(stderr);
        info.device_count = 0;
        return info;
    }

    if (info.device_count == 0) {
        fprintf(stderr, "ggml_sycl_init: no devices found\n"); fflush(stderr);
        return info;
    }

    if (info.device_count > GGML_SYCL_MAX_DEVICES) {
        fprintf(stderr, "ggml_sycl_init: too many devices (%d > %d)\n", info.device_count, GGML_SYCL_MAX_DEVICES); fflush(stderr);
        info.device_count = GGML_SYCL_MAX_DEVICES;
    }

    int64_t total_vram = 0;
    for (int i = 0; i < info.device_count; ++i) {
        try {
            fprintf(stderr, "ggml_sycl_init: initializing device %d\n", i); fflush(stderr);
            ...
            info.default_tensor_split[i] = (float)total_vram;
            total_vram += prop.get_global_mem_size();

            info.devices[i].cc =
                100 * prop.get_major_version() + 10 * prop.get_minor_version();
            info.devices[i].nsm = prop.get_max_compute_units() / 16;
            ...
            fprintf(stderr, "ggml_sycl_init: error initializing device %d: %s\n", i, e.what()); fflush(stderr);
            info.device_count = i;
            break;
        }
    }
    ...
    if (total_vram > 0) {
        for (int id = 0; id < info.device_count; ++id) {
            info.default_tensor_split[id] /= (float)total_vram;
        }
    }
    return info;
}
...
inline void ggml_sycl_op_mul_mat_sycl(
    ...
    if (src0->type == GGML_TYPE_BF16 && !g_ggml_sycl_disable_dnn && ggml_is_contiguous(src0) &&
        ...
        if (src1->type != GGML_TYPE_BF16) {
            ...
            to_bf16_sycl(src1_ddf_i, src1_as_bf16.get(), src1_ncols*ne10, ne10, stream);
        } else {
    ...
    if ((src0->type == GGML_TYPE_F16 || ggml_is_quantized(src0->type)) && use_fp16 && ggml_is_contiguous(src0) &&
        ...
        if (src0->type != GGML_TYPE_F16) {
            ...
            to_fp16_sycl(src0_dd_i, src0_as_f16.get(), ne, ne00, stream);
        }
        ...
        if (src1->type != GGML_TYPE_F16) {
            ...
            to_fp16_sycl(src1_ddf_i, src1_as_f16.get(), ne, ne10, stream);
        }
...
#endif
        {
            ...
            to_fp32_sycl(dst_f16.get(), dst_dd_i, row_diff*src1_ncols, src1_ncols, stream);
        }
    } else {
        ...
        if (src0->type != GGML_TYPE_F32) {
            ...
            to_fp32_sycl(src0_dd_i, src0_ddq_as_f32.get(), row_diff*ne00, ne00, stream);
        }
        if (src1->type != GGML_TYPE_F32) {
            ...
            to_fp32_sycl(src1_ddf_i, src1_ddq_as_f32.get(), src1_ncols*ne10, ne10, stream);
        }
...
static void ggml_sycl_mul_mat_batched_sycl(ggml_backend_sycl_context & ctx, const ggml_tensor * src0,
    ...
    if (src1->type != GGML_TYPE_F16) {
        ...
        const to_fp16_sycl_t to_fp16_sycl = ggml_get_to_fp16_sycl(src1->type, dst);
        GGML_ASSERT(to_fp16_sycl != nullptr);
        to_fp16_sycl(src1_f16, src1_f16_alloc.get(), ne_src1, ne10, queue);
# else
        ...
        case GGML_OP_SET_ROWS:
            {
                return ((op->type == GGML_TYPE_F32 || op->type == GGML_TYPE_F16 || op->type == GGML_TYPE_BF16 ||
                         ...
                         op->type == GGML_TYPE_ISO4 || op->type == GGML_TYPE_ROTOR4) &&
        ...
        case GGML_OP_CPY:
            {
                ...
                if (src0_type == GGML_TYPE_F32 && src1_type == GGML_TYPE_ISO4) {
                    return true;
                }
                if (src0_type == GGML_TYPE_F32 && src1_type == GGML_TYPE_ROTOR4) {
                    return true;
                }
...
ggml_backend_reg_t ggml_backend_sycl_reg() {
...

    {
        ...
        std::lock_guard<std::mutex> lock(mutex);
        if (!initialized) {
            fprintf(stderr, "ggml_backend_sycl_reg: initializing\n"); fflush(stderr);
...
#if defined(GGML_BACKEND_SHARED) && !defined(GGML_BACKEND_DL)
extern "C" GGML_BACKEND_API ggml_backend_reg_t ggml_backend_init(void) {
    return ggml_backend_sycl_reg();
}
#else
