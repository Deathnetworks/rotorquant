...
static void mul_mat_vec_q_reorder(const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    ...
    for (int i = sg.get_local_linear_id() / block_elements_per_subgroup; i < blocks_per_row; i += blocks_per_subgroup) {
        ...
        for (int elem = 0; elem < block_elements_per_subgroup; elem += WARP_SIZE) {
            ...
            partial_sum += reorder_vec_dot_q_sycl()(vx, bx_offset, d_offset, q8_1_quant_ptr, q8_1_ds_ptr, iqs, ibx);
        }
    }
...
static void mul_mat_vec_q(const void * __restrict__ vx, const void * __restrict__ vy, float * __restrict__ dst,
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row; i += blocks_per_warp) {
        ...
        for (size_t elem = 0; elem < qi / vdr; elem += WARP_SIZE) {
            ...
            tmp += vec_dot_q_sycl(&x[ibx], &y[iby], iqs, i);
        }
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq2_xxs_q8_1(&x[ibx], &y[iby], iqs, ibx, iq2xxs_grid, ksigns_iq2xs, kmask_iq2xs);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq2_xs_q8_1(&x[ibx], &y[iby], iqs, ibx, iq2xs_grid, ksigns64);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq2_s_q8_1(&x[ibx], &y[iby], iqs, ibx);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq3_xxs_q8_1(&x[ibx], &y[iby], iqs, ibx, iq3xxs_grid, ksigns64);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq3_s_q8_1(&x[ibx], &y[iby], iqs, ibx, iq3s_grid);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq1_s_q8_1(&x[ibx], &y[iby], iqs, ibx, iq1s_grid_gpu);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq1_m_q8_1(&x[ibx], &y[iby], iqs, ibx);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq4_nl_q8_1(&x[ibx], &y[iby], iqs, ibx);
    }
...
// partial sum for each thread
    ...
    for (int i = item_ct1.get_local_id(2) / (qi / vdr); i < blocks_per_row;
        ...
        tmp += vec_dot_iq4_xs_q8_1(&x[ibx], &y[iby], iqs, ibx);
    }
...
static void mul_mat_vec_iso4_q8_1_sycl(const void * vx, const void * vy, float * dst, const int ncols, const int nrows,
                                       dpct::queue_ptr stream) {
    GGML_ASSERT(ncols % QK4_0 == 0);
    const int block_num_y = (nrows + GGML_SYCL_MMV_Y - 1) / GGML_SYCL_MMV_Y;
    const sycl::range<3> block_nums(1, 1, block_num_y);
...

    {
        stream->submit([&](sycl::handler & cgh) {
            cgh.parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                             [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                                 mul_mat_vec_q<QK4_0, QI4_0, block_iso4, VDR_ISO4_Q8_1_MMVQ, vec_dot_iso4_q8_1>(
                                     vx, vy, dst, ncols, nrows, item_ct1);
                             });
        });
    }
...
static void mul_mat_vec_rotor4_q8_1_sycl(const void * vx, const void * vy, float * dst, const int ncols, const int nrows,
                                         dpct::queue_ptr stream) {
    GGML_ASSERT(ncols % QK4_0 == 0);
    const int block_num_y = (nrows + GGML_SYCL_MMV_Y - 1) / GGML_SYCL_MMV_Y;
    const sycl::range<3> block_nums(1, 1, block_num_y);
...

    {
        stream->submit([&](sycl::handler & cgh) {
            cgh.parallel_for(sycl::nd_range<3>(block_nums * block_dims, block_dims),
                             [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(WARP_SIZE)]] {
                                 mul_mat_vec_q<QK4_0, QI4_0, block_rotor4, VDR_ROTOR4_Q8_1_MMVQ, vec_dot_rotor4_q8_1>(
                                     vx, vy, dst, ncols, nrows, item_ct1);
                             });
        });
    }
    ...
    for (int i = 0; i < src1_ncols; i++) {
        ...
        switch (src0->type) {
            ...
            case GGML_TYPE_NVFP4:
                ...
                break;
            case GGML_TYPE_ISO4:
                mul_mat_vec_iso4_q8_1_sycl(src0_dd_i, src1_ddq_i_bs, dst_dd_i_bs, ne00, row_diff, stream);
                break;
            case GGML_TYPE_ROTOR4:
                mul_mat_vec_rotor4_q8_1_sycl(src0_dd_i, src1_ddq_i_bs, dst_dd_i_bs, ne00, row_diff, stream);
                break;
            default:
...
bool ggml_sycl_mul_mat_vec_q_id(
    ...
    switch (src0_type) {
        ...
        case GGML_TYPE_NVFP4:
            ...
            return true;
        case GGML_TYPE_ISO4:
            launch_mul_mat_vec_q_moe<QK4_0, QI4_0, block_iso4, VDR_ISO4_Q8_1_MMVQ, vec_dot_iso4_q8_1>(
                vx_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used,
                expert_weight_stride, dst_row_stride, src1_row_stride, stream);
            return true;
        case GGML_TYPE_ROTOR4:
            launch_mul_mat_vec_q_moe<QK4_0, QI4_0, block_rotor4, VDR_ROTOR4_Q8_1_MMVQ, vec_dot_rotor4_q8_1>(
                vx_base, vy, ids_dev, dst_base, ncols, nrows, n_experts_used,
                expert_weight_stride, dst_row_stride, src1_row_stride, stream);
            return true;
        default:
