...
static void flash_attn_ext_vec(const char* __restrict__ Q,
    ...
    constexpr bool Q_q8_1 = type_K != GGML_TYPE_F16 && type_K != GGML_TYPE_ISO4 && type_K != GGML_TYPE_ROTOR4;
#ifdef GGML_SYCL_F16
    ...
    // printf("SYCL: fattn_vec_case head_size=%d, type_K=%d, type_V=%d\n", (int)D, (int)type_K, (int)type_V);
    if (type_V == GGML_TYPE_ISO4) {
        // printf("SYCL: Using ISO4 Flash Attention kernel\n");
    }
...
#pragma unroll
        ...
        for (int j = 0; j < ncols; ++j) {
            for (int i = 0; i < (1 > D / (sizeof(int) * nthreads_KQ) ? 1 : D / (sizeof(int) * nthreads_KQ)); ++i) {
                Q_ds[j][i] = sycl::float2(1.0f, 0.0f);
            }
        }
#ifdef GGML_SYCL_F16
...
#pragma unroll
    ...
    if constexpr (type_K == GGML_TYPE_ISO4 || type_K == GGML_TYPE_ROTOR4) {
        for (int j = 0; j < ncols; ++j) {
            bool out_of_bounds = (ncols > 1 && ic0 + j >= int(ne01.z()));
#ifdef GGML_SYCL_F16
            const sycl::half * Q_ptr = (const sycl::half *)(Q + j*nb01);
#else
            const float * Q_ptr = (const float *)(Q + j*nb01);
#endif
            for (int i = tid; i < D; i += nthreads) {
                KQ[j*D + i] = out_of_bounds ? 0.0f : (Q_ptr[i] * scale);
            }
        }
        item_ct1.barrier(sycl::access::fence_space::local_space);
    }
...
#pragma unroll
            for (int j = 0; j < ncols; ++j) {
                const void * q_ptr = (const void *)Q_reg[j];
                if constexpr (type_K == GGML_TYPE_ISO4 || type_K == GGML_TYPE_ROTOR4) {
                    q_ptr = (const void *)&KQ[j*D];
                }
                float sum = vec_dot_KQ(K + i_KQ*nb11, q_ptr, Q_i32[j], Q_ds[j]);
...
#define EXTERN_DECL_FATTN_VEC_CASES(D, type_K)             \
    ...
    extern DECL_FATTN_VEC_CASE(D, type_K, GGML_TYPE_ISO4); \
    extern DECL_FATTN_VEC_CASE(D, type_K, GGML_TYPE_ROTOR4);
...
EXTERN_DECL_FATTN_VEC_CASES( 64, GGML_TYPE_ISO4)
EXTERN_DECL_FATTN_VEC_CASES( 64, GGML_TYPE_ROTOR4)
...
EXTERN_DECL_FATTN_VEC_CASES(128, GGML_TYPE_ISO4)
EXTERN_DECL_FATTN_VEC_CASES(128, GGML_TYPE_ROTOR4)
...
EXTERN_DECL_FATTN_VEC_CASES(256, GGML_TYPE_ISO4)
EXTERN_DECL_FATTN_VEC_CASES(256, GGML_TYPE_ROTOR4)
...
EXTERN_DECL_FATTN_VEC_CASES(512, GGML_TYPE_ISO4)
EXTERN_DECL_FATTN_VEC_CASES(512, GGML_TYPE_ROTOR4)
