...
// Math helpers moved to vecdotq.hpp

template <int D, int nthreads, int warp_size>
static __dpct_inline__ float vec_dot_fattn_vec_KQ_iso4(const char * __restrict__ K_c,
                                                       const void * __restrict__ Q_v,
                                                       const int * __restrict__ Q_q8,
                                                       const void * __restrict__ Q_ds_v) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    const block_iso4 * K_iso4 = (const block_iso4 *) K_c;
    
    const int tid = nthreads == warp_size ? item_ct1.get_local_id(2) : item_ct1.get_local_id(2) % nthreads;
    const int ib = tid / 8; // Each block has 32 elements, 8 threads per block (4 elements each)
    const int it = tid % 8; // Thread within block
    
    if (ib >= D/32) return 0.0f;

    const float d_k = (float)K_iso4[ib].d;
    const sycl::float2 Q_ds = ((const sycl::float2 *) Q_ds_v)[0];
    const float dq = d_k * Q_ds.x();

#ifdef GGML_SYCL_F16
    const sycl::half * Q_ptr = (const sycl::half *) Q_v;
#else
    const float * Q_ptr = (const float *) Q_v;
#endif

    float s, qx, qy, qz;
    get_deterministic_quaternion(ib * 8 + it, &s, &qx, &qy, &qz);

    const int q_offset = ib * 32 + it * 4;
    float q_vec[4] = {
        (float)Q_ptr[q_offset + 0],
        (float)Q_ptr[q_offset + 1],
        (float)Q_ptr[q_offset + 2],
        (float)Q_ptr[q_offset + 3]
    };
    ...
    for (int i = 0; i < 4; ++i) {
        int idx = it*4 + i;
        int ki = (K_iso4[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
...
// Rotor helpers moved to vecdotq.hpp

template <int D, int nthreads, int warp_size>
static __dpct_inline__ float vec_dot_fattn_vec_KQ_rotor4(const char * __restrict__ K_c,
                                                         const void * __restrict__ Q_v,
                                                         const int * __restrict__ Q_q8,
                                                         const void * __restrict__ Q_ds_v) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    const block_rotor4 * K_rotor4 = (const block_rotor4 *) K_c;
    
    const int tid = nthreads == warp_size ? item_ct1.get_local_id(2) : item_ct1.get_local_id(2) % nthreads;
    const int ib = tid / 8; // 8 threads per block
    const int it = tid % 8; // Thread within block
    
    if (ib >= D/32) return 0.0f;

    const float d_k = (float)K_rotor4[ib].d;
    const sycl::float2 Q_ds = ((const sycl::float2 *) Q_ds_v)[0];
    const float dq = d_k * Q_ds.x();
    ...
    // Each block has 10 groups of 3 elements and 2 extra elements (total 32)
    // Distributed among 8 threads of the block.
    ...
    // Group it (0-7)
    {
        ...
        for (int i = 0; i < 3; ++i) {
            int idx = g*3 + i;
            int ki = (K_rotor4[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
            sum += dq * iso4_normalized_centroids[ki] * q_rot[i];
        }
    }

    // Thread 0 and 1 handle Groups 8 and 9
    if (it < 2) {
        int g = 8 + it;
        float s, p12, p13, p23;
        get_deterministic_rotor(ib * 10 + g, s, p12, p13, p23);
    ...
    // Thread 2 handles Extras
    if (it == 2) {
        for (int i = 0; i < 2; ++i) {
            int idx = 30 + i;
            int ki = (K_rotor4[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
            sum += dq * iso4_normalized_centroids[ki] * (float)Q_ptr[ib * 32 + idx];
        }
    }
...
template <typename T, int ne>
static __dpct_inline__ void dequantize_V_iso4(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_iso4 * x = (const block_iso4 *) vx;

    const int64_t ib    =  i0          /  QK4_0;
    const int     local_idx = i0 % QK4_0;

    const float d = (float)x[ib].d;
    
    // ISO4 dequantization with 4D rotation
    // We need to know which group of 4 we are in.
    const int ig = local_idx / 4;
    const int isub = local_idx % 4;

    // To properly de-rotate, we'd need to process all 4 elements in the group.
    // However, dequantize_V is often called per-element or in small SIMD chunks.
    // For ISO4, we MUST apply the inverse rotation.
    
    float s, qx, qy, qz;
    get_deterministic_quaternion((int)ib * 8 + ig, &s, &qx, &qy, &qz);
    
    float dequantized[4];
    for (int i = 0; i < 4; i++) {
        int idx = ig*4 + i;
        int ki = (x[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
        dequantized[i] = d * iso4_normalized_centroids[ki];
    }
    
    float res[4];
    quat_multiply_left(s, -qx, -qy, -qz, dequantized, res);
    
    for (int l = 0; l < ne; l++) {
        // This is tricky because dequantize_V is called with ne elements starting at i0.
        // We assume i0 is aligned or ne is small.
        int cur_local = (i0 + l) % QK4_0;
        int cur_ig = cur_local / 4;
        int cur_isub = cur_local % 4;
        
        // If we crossed a group boundary, we'd need to recompute.
        // For simplicity, we just use the result if it matches our ig.
        if (cur_ig == ig) {
            if constexpr (std::is_same_v<T, float>) {
                ((float *) dst)[l] = res[cur_isub];
            } else {
                ((sycl::half *) dst)[l] = (sycl::half)res[cur_isub];
            }
        } else {
            float s2, qx2, qy2, qz2;
            get_deterministic_quaternion((int)ib * 8 + cur_ig, &s2, &qx2, &qy2, &qz2);
            float deq2[4];
            for (int i = 0; i < 4; i++) {
                int idx = cur_ig*4 + i;
                int ki = (x[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
                deq2[i] = d * iso4_normalized_centroids[ki];
            }
            float res2[4];
            quat_multiply_left(s2, -qx2, -qy2, -qz2, deq2, res2);
            if constexpr (std::is_same_v<T, float>) {
                ((float *) dst)[l] = res2[cur_isub];
            } else {
                ((sycl::half *) dst)[l] = (sycl::half)res2[cur_isub];
            }
        }
    }
}
...
template <typename T, int ne>
static __dpct_inline__ void dequantize_V_rotor4(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_rotor4 * x = (const block_rotor4 *) vx;
    const int64_t ib = i0 / QK4_0;
    const int local_idx = i0 % QK4_0;
    const float d = (float)x[ib].d;

    for (int l = 0; l < ne; l++) {
        int cur_local = (int)((i0 + l) % QK4_0);
        float val;
        if (cur_local < 30) {
            int ig = cur_local / 3;
            int isub = cur_local % 3;
            float s, p12, p13, p23;
            get_deterministic_rotor((int)ib * 10 + ig, s, p12, p13, p23);
            float deq[3];
            for (int i = 0; i < 3; i++) {
                int idx = ig*3 + i;
                int ki = (x[ib].qs[idx/2] >> (4 * (idx%2))) & 0x0F;
                deq[i] = d * iso4_normalized_centroids[ki];
            }
            float res[3];
            rotor_sandwich_vec(s, -p12, -p13, -p23, deq, res);
            val = res[isub];
        } else {
            int ki = (x[ib].qs[cur_local/2] >> (4 * (cur_local%2))) & 0x0F;
            val = d * iso4_normalized_centroids[ki];
        }
        if constexpr (std::is_same_v<T, float>) {
            ((float *) dst)[l] = val;
        } else {
            ((sycl::half *) dst)[l] = (sycl::half)val;
        }
    }
}
...
constexpr vec_dot_KQ_t get_vec_dot_KQ() {
    ...
    } else if constexpr (type_K == GGML_TYPE_ISO4) {
        return vec_dot_fattn_vec_KQ_iso4<D, nthreads, warp_size>;
    } else if constexpr (type_K == GGML_TYPE_ROTOR4) {
        return vec_dot_fattn_vec_KQ_rotor4<D, nthreads, warp_size>;
    } else {
...
constexpr dequantize_V_t get_dequantize_V() {
    ...
    } else if constexpr (type_V == GGML_TYPE_ISO4) {
        return dequantize_V_iso4<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_ROTOR4) {
        return dequantize_V_rotor4<T, ne>;
    } else {
...
void launch_fattn(
    ...
    if (need_f16_K && K->type != GGML_TYPE_F16) {
        ...
        if (ggml_is_contiguously_allocated(K)) {
            ...
            to_fp16(K_data, K_f16.ptr, ggml_nelements(K), K->ne[0], main_stream);
    ...
    if (need_f16_V && V->type != GGML_TYPE_F16) {
        if (V_is_K_view) {
            ...
            if (ggml_is_contiguously_allocated(V)) {
                ...
                to_fp16(V_data, V_f16.ptr, ggml_nelements(V), V->ne[0], main_stream);
